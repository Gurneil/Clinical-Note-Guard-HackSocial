...
def main():
    ...
    out.append(
        ...
        "the full 18-case numbers this document is drawn from; provider/model "
    ...
    out.append(
        "- **`case_02`: a real categorization slip, not a detection miss.** "
        "The two fabricated-symptom claims (fever, nausea) were correctly "
        "flagged by node 3 (entailment) - but `classify_errors_batch` (node "
        "6) labeled them `omission` instead of `fabrication`. The error was "
        "still caught and would still reach a human reviewer; the taxonomy "
        "label attached to it was wrong. Worth fixing before treating "
        "per-category precision as reliable, and left visible here rather "
        "than cleaned up for the writeup.\n"
    )
    out.append(
        ...
        "drug allergy entirely. It is not that the baseline model "
        "\"disagreed\" - a whole-note free-form review, with no structural "
        "requirement to check every transcript fact against the note, "
        "simply has no mechanism that would surface an omission like this. "
        "That structural gap is exactly what node 5 exists to close.\n"
    )
    out.append(
        "- **`case_18`: the honest cost of that same decomposition.** "
        "Breaking a note into many atomic claims/facts (7 checkable items "
        "here) creates 7 independent chances for a false positive instead "
        "of 1. The baseline's conservative, non-decomposed read caught "
        "nothing wrong here (correctly - it's a clean note) partly because "
        "it never atomizes \"denies changes in weight, appetite, sleep, or "
        "energy\" into four separately-judged claims the way the pipeline "
        "does. See `docs/ARCHITECTURE.md`, \"Known limitations\", for the "
        "full discussion.\n"
    )
